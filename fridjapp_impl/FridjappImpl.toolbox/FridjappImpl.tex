...
\@x{}\moduleLeftDash\@xx{ {\MODULE} FridjappImpl}\moduleRightDash\@xx{}%
\@pvspace{8.0pt}%
\@x{ {\EXTENDS} Integers ,\, FiniteSets ,\, Sequences}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANTS} INGREDIENT\_TYPES ,\, MAX\_QTTY ,\, USERS ,\, SERVER}%
\@pvspace{8.0pt}%
\@x{ {\ASSUME} SERVER \.{\in} USERS}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLES} fridj ,\, shoppingList ,\, nRecipesMade ,\, msgs}%
\@pvspace{8.0pt}%
...
\end{cpar}%
\end{lcom}%
 \@x{ Sum ( fun ) \.{\defeq} PT {\bang} ReduceSet ( {\LAMBDA} k ,\, acc \.{:}
 acc \.{+} fun [ k ] ,\,}%
\@x{\@s{129.68} {\DOMAIN} fun ,\, 0 )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Refinement mapping: the number of recipes split by users is summed up.
 The \ensuremath{fridj} of reference is the server\mbox{'}s.
\end{cpar}%
\end{lcom}%
 \@x{ fj \.{\defeq} {\INSTANCE} fridjapp {\WITH} nRecipesMade \.{\leftarrow}
 Sum ( nRecipesMade ) ,\,}%
\@x{\@s{138.44} fridj \.{\leftarrow} fridj [ SERVER ]}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
What\mbox{'}s a \ensuremath{Fridj} and a Shopping List.
\end{cpar}%
\end{lcom}%
\@x{ AllFridjes \.{\defeq} [ INGREDIENT\_TYPES \.{\rightarrow} Nat ]}%
\@x{ AllShoppingLists \.{\defeq} AllFridjes}%
\@pvspace{8.0pt}%
\@x{ FinalUsers \.{\defeq} USERS \.{\,\backslash\,} \{ SERVER \}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
What\mbox{'}s a synchronisation message, and it\mbox{'}s actions.
\end{cpar}%
\end{lcom}%
\@x{ AddAction \.{\defeq}\@w{add}}%
\@x{ RmAction \.{\defeq}\@w{rm}}%
\@x{ Actions \.{\defeq} \{ AddAction ,\, RmAction \}}%
\@x{ FridjObject \.{\defeq}\@w{fridj}}%
\@x{ ShoppingListObject \.{\defeq}\@w{shoppingList}}%
\@x{ Objects \.{\defeq} \{ FridjObject ,\, ShoppingListObject \}}%
\@x{ Messages \.{\defeq} [ user \.{:} FinalUsers ,\,}%
\@x{\@s{62.63} action \.{:} Actions ,\,}%
\@x{\@s{62.63} object\@s{1.86} \.{:} Objects ,\,}%
\@x{\@s{62.63} value \.{:} AllFridjes ]}%
\@pvspace{8.0pt}%
\@x{ TypeOK \.{\defeq}}%
\begin{lcom}{16.4}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Every user\mbox{'}s device is assigned a \ensuremath{fridj} function.
 One of these users is the server.
\end{cpar}%
\end{lcom}%
\@x{\@s{16.4} \.{\land} fridj \.{\in} [ USERS \.{\rightarrow} AllFridjes ]}%
\begin{lcom}{16.4}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Each user has a shopping list as well.
\end{cpar}%
\end{lcom}%
 \@x{\@s{16.4} \.{\land} shoppingList \.{\in} [ USERS \.{\rightarrow}
 AllShoppingLists ]}%
\begin{lcom}{16.4}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
\@x{\@s{16.4} \.{\land} nRecipesMade \.{\in} [ USERS \.{\rightarrow} Nat ]}%
\begin{lcom}{16.4}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The sequence of messages sent to \ensuremath{USERS} by
 \ensuremath{Message.user
}%
\end{cpar}%
\end{lcom}%
 \@x{\@s{16.4} \.{\land} msgs \.{\in} [ USERS \.{\rightarrow} Seq ( Messages )
 ]}%
\@pvspace{8.0pt}%
 \@x{ vars \.{\defeq} {\langle} fridj ,\, shoppingList ,\, nRecipesMade
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{ Min ( a ,\, b ) \.{\defeq} {\IF} a \.{<} b \.{\THEN} a \.{\ELSE} b}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
\@x{ NewMessage ( user ,\, action ,\, object ,\, value ) \.{\defeq}}%
\@x{\@s{16.4} [ user \.{\mapsto} user ,\,}%
\@x{\@s{19.17} action \.{\mapsto} action ,\,}%
\@x{\@s{19.17} object\@s{1.86} \.{\mapsto} object ,\,}%
\@x{\@s{19.17} value \.{\mapsto} value ]}%
\@pvspace{8.0pt}%
\@x{ AddToShoppingListMsg ( user ,\, value ) \.{\defeq}}%
 \@x{\@s{16.4} NewMessage ( user ,\, AddAction ,\, ShoppingListObject ,\,
 value )}%
\@pvspace{8.0pt}%
\@x{ RmFromShoppingListMsg ( user ,\, value ) \.{\defeq}}%
 \@x{\@s{16.4} NewMessage ( user ,\, RmAction ,\, ShoppingListObject ,\, value
 )}%
\@pvspace{8.0pt}%
\@x{ AddToFridjMsg ( user ,\, value ) \.{\defeq}}%
\@x{\@s{16.4} NewMessage ( user ,\, AddAction ,\, FridjObject ,\, value )}%
\@pvspace{8.0pt}%
\@x{ RmFromFridjMsg ( user ,\, value ) \.{\defeq}}%
\@x{\@s{16.4} NewMessage ( user ,\, RmAction ,\, FridjObject ,\, value )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Send messages, that is the value of the \mbox{'}msgs\mbox{'} variable in the
 second
 state of a step.
\end{cpar}%
\end{lcom}%
\@x{ Send ( to ,\, new\_msgs ) \.{\defeq}}%
 \@x{\@s{21.98} msgs \.{'} \.{=} [ msgs {\EXCEPT} {\bang} [ to ] \.{=} @
 \.{\circ} new\_msgs ]}%
\@pvspace{8.0pt}%
\@x{ NotifyServer ( messages ) \.{\defeq}}%
\@x{\@s{16.4} Send ( SERVER ,\, messages )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
\@x{ AddToShoppingList ( user ) \.{\defeq}}%
 \@x{\@s{16.4} \E\, t \.{\in} INGREDIENT\_TYPES ,\, n \.{\in} 1 \.{\dotdot}
 MAX\_QTTY \.{:}}%
 \@x{\@s{27.72} \.{\LET} new\_shopping\_list \.{\defeq} [ shoppingList
 {\EXCEPT} {\bang} [ t ] \.{=} @ \.{+} n ]}%
 \@x{\@s{27.72} \.{\IN} \.{\land} shoppingList \.{'} \.{=}
 new\_shopping\_list}%
 \@x{\@s{48.12} \.{\land} NotifyServer ( {\langle} AddToShoppingListMsg ( user
 ,\, new\_shopping\_list ) {\rangle} )}%
 \@x{\@s{48.12} \.{\land} {\UNCHANGED} {\langle} fridj ,\, nRecipesMade
 {\rangle}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Next, users add bought items in their \ensuremath{fridj} instance.
\end{cpar}%
\end{lcom}%
\@x{ BuyIngredients ( user ) \.{\defeq}}%
\@x{\@s{16.4} \E\, t \.{\in} INGREDIENT\_TYPES \.{:}}%
 \@x{\@s{27.72} \.{\LET} bought\_n \.{\defeq} Min ( MAX\_QTTY \.{-} fridj [ t
 ] ,\, shoppingList [ t ] )}%
 \@x{\@s{48.12} new\_shopping\_list \.{\defeq} [ shoppingList {\EXCEPT}
 {\bang} [ t ] \.{=} @ \.{-} bought\_n ]}%
 \@x{\@s{48.12} new\_fridj \.{\defeq} [ fridj {\EXCEPT} {\bang} [ t ] \.{=} @
 \.{+} bought\_n ]}%
\@x{\@s{27.72} \.{\IN} \.{\land} bought\_n \.{>} 0}%
\@x{\@s{48.12} \.{\land} shoppingList \.{'} \.{=} new\_shopping\_list}%
\@x{\@s{48.12} \.{\land} fridj \.{'} \.{=} new\_fridj}%
 \@x{\@s{48.12} \.{\land} NotifyServer ( {\langle} RmFromShoppingListMsg (
 user ,\, new\_shopping\_list ) ,\,}%
\@x{\@s{122.79} AddToFridjMsg ( user ,\, new\_fridj ) {\rangle} )}%
\@x{\@s{48.12} \.{\land} {\UNCHANGED} {\langle} nRecipesMade {\rangle}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Finally, users cook! They remove items from the \ensuremath{fridj}.
\end{cpar}%
\end{lcom}%
\@x{ MakeRecipe ( user ) \.{\defeq}}%
\@x{\@s{16.4} \E\, r \.{\in} fj {\bang} AllRecipes \.{:}}%
 \@x{\@s{27.72} \.{\LET} new\_fridj \.{\defeq} [ t \.{\in} {\DOMAIN} fridj
 \.{\mapsto} fridj [ t ] \.{-} r [ t ] ]}%
 \@x{\@s{27.72} \.{\land} \A\, t \.{\in} {\DOMAIN} r \.{:} fridj [ t ]
 \.{\geq} r [ t ]}%
\@x{\@s{27.72} \.{\land} fridj \.{'} \.{=} new\_fridj}%
 \@x{\@s{27.72} \.{\land} nRecipesMade \.{'} \.{=} [ nRecipesMade {\EXCEPT}
 {\bang} [ user ] \.{=} @ \.{+} 1 ]}%
 \@x{\@s{27.72} \.{\land} NotifyServer ( {\langle} RmFromFridjMsg ( user ,\,
 new\_fridj ) {\rangle} )}%
\@x{\@s{27.72} \.{\land} {\UNCHANGED} shoppingList}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} \E\, u \.{\in} FinalUsers \.{:}}%
\@x{\@s{16.4} \.{\lor} AddToShoppingList ( u )}%
\@x{\@s{16.4} \.{\lor} BuyIngredients ( u )}%
\@x{\@s{16.4} \.{\lor} MakeRecipe ( u )}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} fridj \.{=} [ t \.{\in} INGREDIENT\_TYPES \.{\mapsto}
 0 ]}%
 \@x{\@s{16.4} \.{\land} shoppingList\@s{6.46} \.{=} [ t\@s{1.86} \.{\in}
 INGREDIENT\_TYPES \.{\mapsto} 0 ]}%
\@x{\@s{16.4} \.{\land} nRecipesMade \.{=} [ u \.{\in} USERS \.{\mapsto} 0 ]}%
 \@x{\@s{16.4} \.{\land} msgs \.{=} [ u \.{\in} USERS \.{\mapsto} {\langle}
 {\rangle} ]}%
\@pvspace{8.0pt}%
\@x{ Spec \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ vars}}%
\@x{ FairSpec \.{\defeq}}%
\@x{\@s{16.4} \.{\land} Spec}%
 \@x{\@s{16.4} \.{\land} \A\, u \.{\in} USERS \.{:} \.{\land} {\WF}_{ vars} (
 BuyIngredients ( u ) )}%
\@x{\@s{96.48} \.{\land} {\WF}_{ vars} ( MakeRecipe ( u ) )}%
\@x{\@s{96.48} \.{\land} {\WF}_{ vars} ( AddToShoppingList ( u ) )}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} Spec \.{\implies} fj {\bang} Spec}%
\@x{ {\THEOREM} FairSpec \.{\implies} fj {\bang} FairSpec}%
\@pvspace{8.0pt}%
 \@x{ TempInv \.{\defeq} {\Diamond} ( \A\, u \.{\in} USERS \.{:} nRecipesMade
 [ u ] \.{>} 0 )}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Jul} 29 17:09:38 \ensuremath{CEST} 2024 by \ensuremath{davd33
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Thu} \ensuremath{Jul} 25
 23:17:45 \ensuremath{CEST} 2024 by \ensuremath{davd33
}%
\end{cpar}%
\end{lcom}%
\end{document}
